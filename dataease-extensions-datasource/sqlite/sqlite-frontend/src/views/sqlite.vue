    ...
    <el-row>
      <el-col>
        ...
        >
          ...
          <el-form-item :label="$t('journal_mode')">
            <el-select v-model="form.configuration.journalMode" filterable
                       :placeholder="$t('please_select')"
                       class="de-select">
              <el-option v-for="item in journalModeArr" :key="item" :label="item" :value="item"/>
            </el-select>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      height: 500,
      disabledNext: false,
      journalModeArr: ['DELETE','WAL','TRUNCATE','PERSIST','MEMORY','OFF']
    }
  },
  computed: {
    form() {
